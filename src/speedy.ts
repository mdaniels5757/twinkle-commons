...
export class Speedy extends SpeedyCore {
	footerlinks = {
		'Speedy deletion policy': 'COM:CSD',
		'CSD prefs': 'COM:TW/PREF#speedy',
		'Twinkle help': 'COM:TW/DOC#speedy',
	};
	...
	validateInputs(): string | void {
		...
		if (csd.has('repost') && input.repost_xfd && !/^(?:commons|com):/i.test(input.repost_xfd)) {
			return 'CSD G4:  The deletion discussion page name, if provided, must start with "Wikipedia:".';
		}
		if (
			csd.has('xfd') &&
			input.xfd_fullvotepage &&
			!/^(?:commons|com):/i.test(input.xfd_fullvotepage)
		) {
			return 'CSD G6 (XFD):  The deletion discussion page name, if provided, must start with "Commons:".';
		}
			...
			list: [
				{
					label:
						'Custom rationale' +
						(Morebits.userIsSysop ? ' (custom deletion reason)' : ' using {{db}} template'),
					value: 'reason',
					code: 'db',
					tooltip:
			...
			list: [
				{
					...
					code: 'g8',
					tooltip:
						'This excludes any page that is useful to the project - in particular, user talk pages.',
				},
			],
		},
		{
			...
			list: [
				...
				{
					label:
						'F7: Fair-use media from a commercial image agency which is not the subject of sourced commentary',
					value: 'badfairuse2',
					code: 'f7',
					tooltip:
				...
				{
					label:
						'F8: File available as an identical or higher-resolution copy on Wikimedia Commons',
					value: 'commons',
					code: 'f8',
					tooltip:
